...
function updatePage() {
        ...
        for (var j = 0; j < tour.length; j++) {
            ...
            // Розділіть рядок дати та отримайте компоненти (день, місяць, рік)
            let dateComponents = match.date.split(".");
            let dayint = parseInt(dateComponents[0], 10);
            let month = parseInt(dateComponents[1], 10) - 1; // Місяці в JavaScript починаються з 0
            let year = parseInt(dateComponents[2], 10);

            // Створіть об'єкт Date
            let dateobj = new Date(year, month, dayint);

            // Масив з назвами днів тижня
            let daysOfWeek = ['неділя', 'понеділок', 'вівторок', 'середа', 'четвер', 'пятниця', 'субота'];

            // Отримайте номер дня тижня (0 - неділя, 1 - понеділок, і так далі)
            let dayOfWeekNumber = dateobj.getDay();

            // Отримайте назву дня тижня
            let dayOfWeekName = daysOfWeek[dayOfWeekNumber];
                ...
                dayInfoDiv.innerHTML = "<p>" + dayOfWeekName+", "+formattedDate + "</p>";
